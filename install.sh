...
# Codex Ralph - Global Installation Script (Codex fork)
set -e

# Configuration
INSTALL_DIR="$HOME/.local/bin"
RALPH_HOME="$HOME/.codex-ralph"
...
exec "$RALPH_HOME/codex_loop.sh" "$@"
EOF

    # Create codex-monitor command
    cat > "$INSTALL_DIR/codex-monitor" << 'EOF'
#!/bin/bash
# Ralph Monitor - Global Command

RALPH_HOME="$HOME/.codex-ralph"

exec "$RALPH_HOME/codex_monitor.sh" "$@"
EOF
...
EOF
    ...
    # Create codex-import command
    cat > "$INSTALL_DIR/codex-import" << 'EOF'
#!/bin/bash
# Ralph PRD Import - Global Command

RALPH_HOME="$HOME/.codex-ralph"

exec "$RALPH_HOME/codex_import.sh" "$@"
EOF

    # Copy actual script files to Ralph home with modifications for global operation
    cp "$SCRIPT_DIR/codex_monitor.sh" "$RALPH_HOME/"
    
    # Copy PRD import script to Ralph home
    cp "$SCRIPT_DIR/codex_import.sh" "$RALPH_HOME/"
    
    # Make all commands executable
    chmod +x "$INSTALL_DIR/ralph"
    chmod +x "$INSTALL_DIR/codex-monitor" 
    chmod +x "$INSTALL_DIR/ralph-setup"
    chmod +x "$INSTALL_DIR/codex-import"
    chmod +x "$RALPH_HOME/codex_monitor.sh"
    chmod +x "$RALPH_HOME/codex_import.sh"
    chmod +x "$RALPH_HOME/lib/"*.sh

    log "SUCCESS" "Ralph scripts installed to $INSTALL_DIR"
}

# Install global codex_loop.sh
install_ralph_loop() {
    log "INFO" "Installing global codex_loop.sh..."
    
    # Create modified codex_loop.sh for global operation
    sed \
        -e "s|RALPH_HOME=\"\$HOME/.codex-ralph\"|RALPH_HOME=\"\$HOME/.codex-ralph\"|g" \
        -e "s|\$script_dir/codex_monitor.sh|\$RALPH_HOME/codex_monitor.sh|g" \
        -e "s|\$script_dir/codex_loop.sh|\$RALPH_HOME/codex_loop.sh|g" \
        "$SCRIPT_DIR/codex_loop.sh" > "$RALPH_HOME/codex_loop.sh"
    
    chmod +x "$RALPH_HOME/codex_loop.sh"
    
    log "SUCCESS" "Global codex_loop.sh installed"
}
...
echo "  4. Monitor: codex-monitor (if running manually)"
EOF
...
main() {
    ...
    echo "  codex-import prd.md     # Convert PRD to Ralph project"
    echo "  codex-monitor           # Manual monitoring dashboard"
    echo ""
    echo "Quick start:"
...
case "${1:-install}" in
    ...
    uninstall)
        ...
        rm -f "$INSTALL_DIR/ralph" "$INSTALL_DIR/codex-monitor" "$INSTALL_DIR/ralph-setup" "$INSTALL_DIR/codex-import"
        rm -rf "$RALPH_HOME"
