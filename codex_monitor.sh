...
# Codex Ralph Status Monitor - Live terminal dashboard for the Codex Ralph loop
set -e

STATUS_FILE="status.json"
LOG_FILE="logs/codex-ralph.log"
REFRESH_INTERVAL=2
...
display_status() {
    ...
    if [[ -f "$STATUS_FILE" ]]; then
        ...
        echo -e "${RED}│${NC} Status file not found. Codex Ralph may not be running."
        echo -e "${RED}└─────────────────────────────────────────────────────────────────────────┘${NC}"
        echo
    fi
...
# Main monitor loop
main() {
    echo "Starting Codex Ralph Monitor..."
    sleep 2
