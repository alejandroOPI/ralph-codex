...
CLAUDE_TIMEOUT_MINUTES=15  # Default: 15 minutes timeout for Codex execution
...
setup_tmux_session() {
    local session_name="codex-ralph-$(date +%s)"
...
main() {
    ...
    if [[ ! -f "$PROMPT_FILE" ]]; then
        ...
        echo ""
        echo "To fix this:"
        echo "  1. Create a new project: codex-ralph-setup my-project"
...
IMPORTANT: This command must be run from a Ralph project directory.
           Use 'codex-ralph-setup project-name' to create a new project first.
...
Example workflow:
    codex-ralph-setup my-project     # Create project
